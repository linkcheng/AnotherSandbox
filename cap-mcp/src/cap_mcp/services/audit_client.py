"""cap-mcp 审计上报（fire-and-forget）。contracts/audit-ingest.md, research.md R9。

读 env ORCHESTRATOR_URL/WORKSPACE_ID（docker-compose.workspace.yml.tmpl 注入）。
MCP 工具调用层无 HTTP header，actor 暂为 None（agent 操作）；未配置则静默跳过。
"""
...
logger = logging.getLogger("cap_mcp.audit")
