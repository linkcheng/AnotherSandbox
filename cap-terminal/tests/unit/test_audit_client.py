"""T060(cap-terminal): audit_client fire-and-forget 测试。research.md R9。"""
import asyncio
...
from cap_terminal.services.audit_client import AuditClient


def test_report_no_config_skips_silently():
    AuditClient(orch_url="", workspace_id="").report("shell.exec", {}, actor_user_id=None, success=True)
...
async def test_report_fires_and_forgets():
    ...
    with patch("httpx.AsyncClient.post", new=fake_post):
        c.report("shell.exec", {"command": "echo"}, actor_user_id="u", success=True)
...
async def test_report_swallows_errors_best_effort(caplog):
    ...
    with patch("httpx.AsyncClient.post", new=AsyncMock(side_effect=httpx.ConnectError("refused"))):
        c.report("shell.exec", {}, actor_user_id=None, success=False)
